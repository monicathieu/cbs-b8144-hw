...
acs <- read_csv("https://www.jaredlander.com/data/acs_ny.csv") %>% 
  mutate(across(-c(Acres, FamilyType, NumUnits, OwnRent, YearBuilt, FoodStamp, HeatingFuel, Language), as.numeric))
```
...
recipe_q1 <- recipe(FamilyIncome ~ ., data = acs) %>% 
  step_rm(FoodStamp) %>% 
  step_dummy(all_nominal(), one_hot = TRUE)
```

```{r}
cv_spec_q1 <- vfold_cv(acs,
                       v = 10)
```


_Fit the model:_ Fit the model using functions from the `tidymodels` packages. Set `glmnet` as your model engine. Set `mixture` to the correct value to run a pure lasso regression.

```{r q1-1-set-model}
# Code that fits model here
model_q1 <- linear_reg(penalty = 1, mixture = 1) %>% 
  set_engine("glmnet")

model_q1_test <- linear_reg() %>% 
  set_engine("lm")
```
...
```{r}
fit_q1 <- fit(workflow_q1, data = acs)
```
...
```{r}
glmnet_q1 <- extract_fit_engine(fit_q1)
```
...
Prep the data: Filter the data to include plays from just one team, where PlayType is either "RUSH" or "PASS". Convert values of "RUSH" and "PASS" in `PlayType` to 0 and 1 respectively.
