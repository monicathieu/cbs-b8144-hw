...
Using the `diamonds` data from the `ggplot2` package, calculate the mean for each of the numeric columns using `summarize()`, with `across()`, and `where()` to calculate for the numeric columns without having to select them by name. Display the resulting data.
...
```{r}
diamonds %>% map_if(is.numeric, mean, .else = ~NULL)
```
...
```{r get-gdp}
gdp_data <- WDI::WDI_data$country %>% 
  as_tibble() %>% 
  filter(region != "Aggregates") %>% 
  sample_n(5) %>% 
  pull(iso2c) %>% 
  WDI::WDI(start = 2000, end = 2016) %>% 
  pivot_wider(names_from = year, values_from = NY.GDP.PCAP.KD, names_prefix = "gdp.")
```
...
Using the `airquality` data from the `datasets` package, use `pivot_longer()` from `tidyr` to convert the data from wide format to long format. As shown in the solution output, one column should list the levels "Ozone", "Solar.R", "Wind" and "Temp", and another column should list the corresponding values. "Month" and "Day" should be their own columns. Display the first 10 rows of the resulting data.
...
```{r q8, exercise = TRUE, exercise.setup = "get-gdp"}
gdp_data
```
